  ...
  async run(): Promise<void> {
    try {
      ...
      if (!this.flags["app-uid"]) {
        app = await getInstalledApps(this.flags, this.sharedConfig.org, {
      ...
      await strategy.run(
        ...
        appType,
        this.marketplaceAppSdk
      );

      this.log(
        this.$t(uninstallAppMsg.APP_UNINSTALLED, {
          app: app?.name || this.flags["app-uid"],
        }),
        "info"
      );
    } catch (error: any) {
