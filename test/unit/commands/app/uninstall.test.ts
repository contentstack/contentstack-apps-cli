...
describe("app:uninstall", () => {
  describe("Uninstall an app from organization", () => {
    test
      ...
      .nock(`https://${developerHubBaseUrl}`, (api) =>
        api.get(`/manifests/${mock.apps[1].uid}/installations`).reply(200, {
          data: mock.installations,
        })
      )
      .nock(`https://${developerHubBaseUrl}`, (api) =>
        api.get(`/manifests/${mock.apps[0].uid}/installations`).reply(200, {
          data: mock.installations,
        })
      )
      .nock(`https://${developerHubBaseUrl}`, (api) =>
        api.delete(`/installations/${mock.installations[1].uid}`).reply(200, {
          data: {},
        })
      )
      .command([
  ...
  describe("Uninstall an app from a stack", () => {
    test
      ...
      .nock(`https://${developerHubBaseUrl}`, (api) =>
        api.delete(`/installations/${mock.installations[0].uid}`).reply(200, {
          data: {},
        })
      )
      .command([
  ...
  describe("Fail to uninstall an app from a stack", () => {
    test
      ...
      .nock(`https://${developerHubBaseUrl}`, (api) =>
        api.delete(`/installations/wrong-uid`).replyWithError({
          status: 404,
          message: "App with id wrong-uid not installed",
          error: "Not Found",
        })
      )
      .command(["app:uninstall", "--installation-uid", "wrong-uid"])
      .exit(1)
      .do(({ stdout }) => {
